...
| 2026-06-30 | Technical analysis panel (2 commits, TDD). **Commit 1** (`14084f9`): `detect_technical_signals()` now returns `technical_type` / `specs` / `compare_on` (value-anchored — never infers a spec value). Fixed a real product bug: `_find_membrane` used naive substring `in`, so "durable water re**pe**llent" matched **ePE** membrane and "pr**event**ion" matched **eVent** → phantom membrane signal → ordinary DWR jackets wrongly suppressed. Fix: word-boundary regex. Patched insulated-jacket detection gap: shared `_find_fill_power()` regex between detector + extractor (can't drift); explicit fill-power figure (e.g. "800-fill down") counts as the insulation signal but fill power **alone** is not technical (still 2-category threshold); generic "puffer"/"down jacket"/"RDS-certified" don't count; `_classify_type` reordered so insulation dominates a bare shell membrane. `_INSULATION_TERMS`→`_INSULATION_BRANDS` (PrimaLoft/Thermolite/Polartec only). **Commit 2** (`e4895c2`): render the panel — app.py attaches full `technical_analysis` dict (kept `technical_override` for back-compat); new `/TECHNICAL ANALYSIS` panel in index.html (dotted-leader spec rows + type-aware "Compare next:" line) replaces old static override copy; app.js `techSubline(type)` varies subline by `technical_type` (shell→weather, insulated→warmth, midlayer, general fallback — a shell is NEVER told to compare on insulation), `renderTechnicalPanel()` renders only present specs, reset clears panel; style.css drops `.technical-override*`/`.technical-signals-list*`, adds `.technical-compare`/`.technical-compare-label`. 48 engine + 59 pytest passing. Verified 3 render cases locally (waterproof shell, insulated jacket, non-technical unchanged). Pushed to Railway. |
